""" Base api client that implements basic methods to query the Podman remote api. """
import json
import struct
...
class BaseAPIClient(Session):
    ...
    Args:
        base_url (str): URL to the Podman server. For example, ``unix:///run/podman/podman.sock``
    ...
    def _retrieve_server_version(self):
        try:
            ...
            raise PodmanException(
                'Invalid response from podman remote api: key "ApiVersion"'
                ' is missing.'
            )
    ...
    def _url(self, pathfmt, *args, **kwargs):
        ...
        if kwargs.get('versioned_api', True):
            return '{0}/v{1}/libpod{2}'.format(
                self.base_url, self._version, pathfmt.format(*args)
            )
        else:
            return '{0}/libpod{1}'.format(self.base_url, pathfmt.format(*args))
    ...
    def _post_json(self, url, data: dict, **kwargs):
    ...
    def reload_config(self, dockercfg_path=None):
        ...
        Args:
            dockercfg_path (str): Use a custom path for the Podman config file
        ...
        TODO: What is the equivalent to the Docker config file for Podman?
        """
        self._auth_configs = auth.load_config(
